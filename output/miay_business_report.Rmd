...
**Medium** describes *how* a user arrived at madeinamericayarns.com: direct, referral, and search engine. 
If the medium is **direct**, the user typed "madeinamericayarns.com" directly into their browser's address bar. If the medium is **referral**, the user reached MiAY through another website that linked to MiAY. If the medium is **search engine**, the user found MiAY through a search engine.
...
df <- traffic %>% 
  ...
  mutate(medium = str_replace(replace_na(medium,"direct"), "organic", "search engine")) %>%
...
y_max <- max(df$newsletter_sign_up) 

cols <- c("search engine" = "slategray",
          "referral" = "slateblue1",
          "direct" = "slateblue4")
...
df <- traffic %>% 
  ...
  #format source/medium for label
  mutate("sessions" = n(),
         medium = factor(str_replace(replace_na(medium, "direct"), "organic", "search engine"),
                         levels = c("direct", "referral", "search engine"))) %>%
  ungroup() %>%
  filter(sessions >= 10) %>%
  select(source,
         medium, 
...
df$source <- factor(df$source, 
                           levels = df$source[order(df$sessions, decreasing = TRUE)])
...
df %>%
  ggplot(aes(x = source, y = sessions, fill=medium)) + 
  geom_col(alpha = 0.8, width = 0.5) +
